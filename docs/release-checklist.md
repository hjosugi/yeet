...
The source tree targets Yeet 0.5.3. Metadata that does not depend on the final
tag is updated before tagging and is checked by `cargo metadata` and AppStream
validation.

## Complete before tagging v0.5.3
...
- [x] `packaging/arch/PKGBUILD` source-archive SHA-256 for `v0.5.3.tar.gz`, then
